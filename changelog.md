...
- POJO-facade entity representation (now the only representation): each entity maps to a generated Java facade whose accessors delegate to the BoxLang instance, unlocking `uuid` (and other) id generators, composite ids, `byte[]`, and full metamodel access. Developers still only handle the BoxLang class.
...
- Associations and lazy proxies in the facade representation — developers only ever see BoxLang instances, never facades.
- Modern Hibernate 7 `mapping.xml` output via `MappingXMLWriter` (the only mapping format): ids/generators, converters, version, discriminators, single-table + joined inheritance, `many-to-one`/`one-to-many`/`many-to-many`, composite ids, formulas, and second-level cache.
...
- `DEVELOPER.md`: developer guide to the Hibernate 5 → 7 migration, with architecture diagrams and design decisions.
...
- `entityDelete()` / `entityMerge()` operate on the managed facade, and `ormEvictEntity()` evicts under the Hibernate entity-name.
...
- `uuid`-generated ids round-trip through a primary-key load.
- `removeX()` on a to-many removes by identity/equality, not by the owner's id property names.
- A to-many association can be structurally modified while iterating its getter (stable snapshot).
- Updating an entity with an association/collection no longer double-fires `preUpdate`/`postUpdate`.
- Completed the facade wrap/unwrap boundary across the remaining BIFs (`entityReload`, `entityIsAttached`, `entityLoadByExample`, association query parameters).
