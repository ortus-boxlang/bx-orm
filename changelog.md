...
- POJO-facade entity representation, now the **default** entity representation and controllable per application via the `entityFacades` ORM setting (default `true`). Each entity is mapped to a generated real Java "facade" class (in `ortus.boxlang.modules.orm.hibernate.facade`) whose accessors delegate their state to the BoxLang instance, so Hibernate sees a real class and a real id member — which unlocks `uuid` (and other) id generation that is impossible for class-less dynamic (MAP) entities. BoxLang developers still only ever handle the BoxLang class; the facade is wrapped/unwrapped at the BIF boundary. Setting `entityFacades = false` restores the byte-for-byte original class-less MAP behavior. A dedicated Derby smoke test (`FacadeRepresentationBootTest`) boots a facade-mode app and round-trips a uuid-keyed entity through `entityNew`/`entitySave`/`entityLoadByPK`/`ormExecuteQuery`.
