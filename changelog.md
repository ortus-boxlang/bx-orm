...
- Value/element collections (`fieldtype="collection"`) are now supported and emitted as a JPA `<element-collection>`: a collection of scalar values (an array today) held in a separate collection table joined back to the owner. Previously a `collection` property was silently dropped (it was neither a column, association, nor version, and the code carried only a dead "not yet supported" note). The element value is declared as a `<column>` plus an explicit Hibernate `JavaType` descriptor (`<java-type>`), since the modern element-collection schema makes `<column>` and `<convert>` mutually exclusive. A `HibernateFeatureCoverageBootTest` round-trips an array-of-strings element collection on Derby in facade mode.
