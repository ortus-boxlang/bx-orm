...
- New `MappingXMLWriter` that emits Hibernate 7's modern `mapping.xml` format (root `<entity-mappings>` in namespace `http://www.hibernate.org/xsd/orm/mapping`, version `7.0`) as an alternative to the legacy `hbm.xml` DTD writer. This is Hibernate-8 readiness (`hbm.xml` is deprecated-for-removal). Opt in per application via the `ormXmlMapping` ORM setting (default `false`, so production stays on the HBM writer). BoxLang-facing ORM behavior is identical; only the emitted mapping XML differs. Dynamic (MAP) entities are emitted as `<entity name="..." metadata-complete="true">` with no `class` attribute; the writer covers ids/`increment` generators, JPA `AttributeConverter`s (`<convert>`), `<version>`, discriminators, single-table inheritance (including a per-subclass secondary join table via `<secondary-table>`), joined inheritance, `many-to-one`/`one-to-many`/`many-to-many` associations, composite ids, formulas, and second-level cache. A dedicated Derby smoke test (`DerbyOrmXmlMappingBootTest`) boots an app with the flag on and round-trips these constructs. The modern writer supports both representations: in the default facade mode it emits the entity's generated facade class (`<entity name="Order" class="...OrderFacade">`, and the facade class `name` on `<subclass>`/`<joined-subclass>` and association `target-entity`), so `uuid` (and other class-dependent) id generation works; in MAP mode it emits class-less `<entity name="..." metadata-complete="true">` entities, for which `uuid` id generation and `byte[]`/`binary` basic attributes remain inexpressible in the modern format (documented with `TODO`s in the writer). Because the modern format binds the mapped class via the JPA annotation path, a facade entity's Hibernate entity-name is its facade class FQN and the BoxLang name is the JPA/HQL import; by-name Session/metamodel calls are routed through the import name and `BoxEntityNameResolver` reports the facade FQN. The `derbyOrmXmlApp` test fixture runs the modern writer in facade mode.
