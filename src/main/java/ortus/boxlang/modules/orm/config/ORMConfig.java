...
public class ORMConfig {
	 ...
	 * regenerate at runtime. The manifest is generated by an {@code auto}-mode boot.</li>
	 * </ul>
	 */
	public String								ormManifest				= "off";

	/**
	 * ORM manifest location - the directory that holds the {@code .bxorm/} boot cache folder.
	 * <p>
	 * Blank (default) means the application root. A relative path is resolved against the application root; an absolute
	 * path is used as-is. The folder name itself is always {@code .bxorm}; only its parent directory moves. The
	 * {@code bxorm} CLI mirrors this with its {@code --dir=<path>} flag.
	 */
	...
	private void process( IStruct properties, IBoxContext context ) {
		...
		if ( properties.containsKey( ORMKeys.ormManifestLocation ) && properties.get( ORMKeys.ormManifestLocation ) != null ) {
			manifestLocation = StringCaster.cast( properties.get( ORMKeys.ormManifestLocation ) ).trim();
		}
