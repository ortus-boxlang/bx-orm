...
public class ORMConfig {
	...
	/**
	 * Specifies whether ColdFusion should automatically generate entity mappings for the persistent CFCs. If autogenmap=false, the mapping should be
	 ...
	 *
	 * Use transaction blocks in order to demarcate your regions that should start, flush and end a transaction.
	 */
	private boolean	autoManageSession;

	/**
	 * Specifies the location of the configuration file that the secondary cache provider should use. This setting is used only when
	 ...
	 * <p>
	 * <strong>Important:</strong> If it is not set, the extension looks at the application directory, its sub-directories, and its mapped directories to
	...
	/**
	 * <ul>
	 * <li><code>update</code> : Creates the database according to your ORM model. It only does incremental updates. It will never remove tables, indexes,
	 * etc.</li>
	 * <li><code>dropcreate</code> : Same as above but it destroys the database if it has ny content and recreates it every time the ORM is reloaded.</li>
	...
	/**
	 * The dialect to use for your database. By default Hibernate will introspect the datasource and try to figure it out. See the dialects section below.
	 *
	...
	/**
	 * If true, then it enables the ORM event callbacks in entities and globally via the `eventHandler` property.
	 */
	...
	/**
	 * Specifies if an orm flush should be called automatically at the end of a request. In our opinion this SHOULD never be true. Database persistence
	 ...
	 * - smart : This strategy changes the logical table or column name to uppercase.
	...
	/**
	 * If enabled, the ORM will create the Hibernate mapping XML (*.hbmxml) files alongside the entities. This is great for debugging your entities and
	 * relationships.
	 *
	 */
	...
	/**
	 * The default database schema to use for database connections. This can be overriden at the datasource level.
	 */
	private String	schema;

	/**
	 * Specifies the default Database Catalog that ORM should use. This can be overriden at the datasource level.
	 */
