...
/**
 * Boots a full ORM application/SessionFactory against a real MariaDB datasource, and runs a
 * save/query CRUD smoke check.
 * <p>
 * This is a standalone boot test (does not extend {@code tools.BaseORMTest}) because it uses its own,
 * isolated BoxLang application (`src/test/resources/mariadbApp`) with a MariaDB datasource (via the real
 * MariaDB JDBC driver, not MySQL Connector/J), rather than the shared MySQL application every other ORM
 * test boots against.
 * <p>
 * It requires a running MariaDB server, so it is gated on the {@code ORM_TEST_MARIADB} environment
 * variable: it is skipped locally (where no server is expected) and runs in CI where a MariaDB service
 * container is provided. Connection details are read from {@code MARIADB_HOST}/{@code MARIADB_PORT}/
 * {@code MARIADB_DB}/{@code MARIADB_USER}/{@code MARIADB_PASSWORD} (see the app descriptor) with sensible
 * localhost defaults.
 */
@EnabledIfEnvironmentVariable( named = "ORM_TEST_MARIADB", matches = "true" )
@TestInstance( TestInstance.Lifecycle.PER_CLASS )
public class MariaDBDialectBootTest {
	...
	public void setUp() {
		...
		context.loadApplicationDescriptor( Paths.get( "src/test/resources/mariadbApp/index.bxs" ).toAbsolutePath().toUri() );
	...
	public void teardown() {
		...
		instance.getApplicationService().shutdownApplication( Key.of( "BXORMMariaDBTest" ) );
	}

	@DisplayName( "It can boot an ORM SessionFactory against a MariaDB datasource, and save/query an entity" )
	@Test
	public void testMariaDBORMBoot() {
		// @formatter:off
